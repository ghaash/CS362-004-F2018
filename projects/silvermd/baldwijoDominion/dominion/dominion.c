...
int AdventurerCard(struct gameState *state, int drawntreasure, int currentPlayer, int temphand[], int z){
   ...
   int cardDrawn;
	...
	if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
	  drawntreasure += 2; //change to+=2 from ++
	else{
...
// Effect of council_room card
void cardCouncil_room(int currentPlayer, struct gameState *state, int handPos){
      ...
      for (int i = 0; i < 4; i++)
	{
      ...
      //+1 Buy
      state->numBuys += 2;  //change from ++ to += 2
...
// Effect of great_halls card
void cardGreat_halls(int currentPlayer, struct gameState *state, int handPos){
      //+1 Card
      drawCard(currentPlayer, state);
			
      //+1 Actions
      state->numActions--;  //change to -- from ++
			
      //discard card from hand
      discardCard(handPos, currentPlayer, state, 0);
}

// Effect of village card
void cardVillage(int currentPlayer, struct gameState *state, int handPos){
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  int drawntreasure=0;
  //int cardDrawn; moved to AdventurerCard function in assign2 refactor 
  ...
  switch( card ) 
    ...
    case council_room:
      ...
      cardCouncil_room(currentPlayer, state, handPos);
    ...
    case great_hall:
      ...
      cardGreat_halls(currentPlayer, state, handPos);
